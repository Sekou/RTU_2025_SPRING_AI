...
class Cart:
    def __init__(self):
        ...
        self.control=0
        self.y=0
        self.gamma=0.01 #угол балки
        self.dgamma_dt=0 #скорость падения
        self.dgamma_d2t=0 #ускорение падения
    ...
    def draw(self, screen):
        ...
        s,c=math.sin(self.gamma+math.pi/2), math.cos(self.gamma+math.pi/2)
    ...
    def sim(self, dt):
        ...
        self.dgamma_d2t=0.9*self.gamma + 0.01*self.a
        self.dgamma_dt+=self.dgamma_d2t * dt
        self.gamma += self.dgamma_dt * dt
        maxAng=1
        if self.gamma>maxAng: self.gamma=maxAng
        if self.gamma<-maxAng: self.gamma=-maxAng
    ...
    def controlExpert(self):
        ...
        if self.gamma<-0.01:
            self.control=100
            if dxCart>0 and abs(self.gamma)>0.3:  self.control+= 0.1*dxCart
        elif self.gamma>0.01:
            self.control=-100
            if dxCart<0 and abs(self.gamma)>0.3:  self.control+= 0.1*dxCart
...
if __name__ == "__main__":
    ...
    while True:
        ...
        gammaDiscr=discretize(cart.gamma, [[-math.pi/2,-0.03],[-0.03,0.03],[0.03,math.pi/2]], [0, 1, 2])
        state = 10 * xDiscr + gammaDiscr
        action=discretize(cart.v, [[-300,-20],[-20,20],[20,300]], [0, 1, 2])
        reward=1/(5*abs(cart.gamma)+1)
        ...
        #3
        drawText(screen, 5, 45, f"gamma={cart.gamma:.2f}")
        #4
        drawText(screen, 5, 65, f"gamma*={gammaDiscr}")
        #5
...
#fix reward //account for dgamma_dt
#fix horizon
